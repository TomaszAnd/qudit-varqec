...
"""Train VarQEC ((5,4,3))_4 with native trapped-ion gates, distance 3."""
import argparse
import warnings
...
def main():
    parser = argparse.ArgumentParser(description="Train native ((5,4,3))_4 code")
    parser.add_argument("--n_steps", type=int, default=2000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--n_layers", type=int, default=2)
    parser.add_argument("--lr", type=float, default=0.05)
    parser.add_argument("--batch_fraction", type=float, default=0.15)
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    N_QUDIT, D, DISTANCE = 5, 4, 3
    K = D
    N_LAYERS = args.n_layers

    E_det, E_corr = build_native_error_set(N_QUDIT, D, DISTANCE)
    print(f"E_det={len(E_det)}, E_corr={len(E_corr)}, dim={D**N_QUDIT}")

    encoder, connections, params_per_layer = create_native_encoder(N_QUDIT, D)
    ...
    save_path = os.path.join(save_dir,
                             f"native_d4_dist{DISTANCE}_{N_LAYERS}layer_seed{args.seed}.npz")

    print(f"Training: native (({N_QUDIT},{K},{DISTANCE}))_4, {N_LAYERS} layer(s), "
    ...
    for step in range(args.n_steps):
        ...
        if step >= 200:
            window_old = np.mean(losses[-200:-100])
            window_new = np.mean(losses[-100:])
