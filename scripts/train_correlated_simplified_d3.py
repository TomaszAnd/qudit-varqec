#!/usr/bin/env python3
"""
Train VarQEC correlated-simplified code, distance 3 (3 layers).

Uses detection-style loss (VarQEC paper Eq. 16) — single sum over E_det,
no quadratic E_a†E_b products (was 625M pairs with old loss).
"""
import argparse
import warnings
...
from src.kl_loss_fast import kl_loss_detection_diagonal_minibatch, save_varqec_result


def main():
    parser = argparse.ArgumentParser(description="Train correlated-simplified d=3 code (detection loss)")
    parser.add_argument("--n_steps", type=int, default=1000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--n_layers", type=int, default=3)
    parser.add_argument("--batch_fraction", type=float, default=0.01)
    ...
    n_sample = max(1, int(len(E_det) * BATCH_FRACTION))
    print(f"Detection loss: sampling {n_sample}/{len(E_det)} ops per step (no E_a†E_b pairs)")
    ...
    def loss_fn(params):
        return kl_loss_detection_diagonal_minibatch(
    ...
    for step in range(args.n_steps):
        ...
        if (not lr_switched) and loss_val < 0.01:
    ...
    save_varqec_result(save_path, theta, losses, "correlated_simplified", DISTANCE, N_LAYERS,
                       metadata={"K": K, "n_qudits": N_QUDITS, "dim_qudit": DIM_QUDIT,
                                 "seed": args.seed, "eta": ETA, "n_max": N_MAX,
                                 "batch_fraction": BATCH_FRACTION, "detection_loss": True})
