...
"""Train VarQEC qutrit code ((3,3,2))_3, distance 2. Native trapped-ion gates."""
import argparse
import warnings
...
def main():
    parser = argparse.ArgumentParser(description="Train qutrit ((3,3,2))_3 code")
    ...
    DIM_QUTRIT = 3
    DISTANCE = 2
    K = DIM_QUTRIT  # 3 codewords
    N_LAYERS = args.n_layers

    E_det, E_corr = build_qutrit_error_set(N_QUTRIT, DISTANCE)
    print(f"E_det={len(E_det)}, E_corr={len(E_corr)}")

    encoder, connections, params_per_layer = create_native_encoder(N_QUTRIT, DIM_QUTRIT)
    print(f"Params per layer: {params_per_layer}, connections: {connections}")

    # distance=2 -> no Term 2, M_products unused
    M_products = []
    ...
    save_path = os.path.join(save_dir,
                             f"qutrit_d{DISTANCE}_{N_LAYERS}layer_seed{args.seed}.npz")

    print(f"Training: qutrit ((3,3,2))_3, d={DISTANCE}, {N_LAYERS} layer(s), "
    ...
    save_varqec_result(save_path, theta, losses, "qutrit_hardware", DISTANCE, N_LAYERS,
                       metadata={"K": K, "n_qutrit": N_QUTRIT, "dim_qutrit": DIM_QUTRIT,
