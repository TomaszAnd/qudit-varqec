...
"""Train VarQEC ((5,5,2))_5 with native trapped-ion gates, distance 2. Feasibility test."""
import argparse
import warnings
...
def main():
    parser = argparse.ArgumentParser(description="Train native ((5,5,2))_5 code")
    ...
    save_path = os.path.join(save_dir,
                             f"native_d5_dist{DISTANCE}_{N_LAYERS}layer_seed{args.seed}.npz")

    print(f"Training: native (({N_QUDIT},{K},{DISTANCE}))_5, {N_LAYERS} layer(s), "
    ...
    for step in range(args.n_steps):
        ...
        if step > 0 and step % 200 == 0:
            save_varqec_result(save_path, theta, losses, "native_hardware_d5", DISTANCE, N_LAYERS,
