#!/usr/bin/env python3
"""Train VarQEC depolarizing code, distance 2 (2 layers). ~15 min."""
import argparse
import warnings
...
def main():
    parser = argparse.ArgumentParser(description="Train depolarizing d=2 code")
    ...
    E_det, E_corr = build_error_sets(N_QUDITS, DISTANCE)
    ...
    save_path = os.path.join(save_dir, f"depolarizing_d{DISTANCE}_{N_LAYERS}layer_seed{args.seed}.npz")

    print(f"Training: depolarizing d={DISTANCE}, {N_LAYERS} layers, seed={args.seed}, {args.n_steps} steps")
    t0 = time.time()
