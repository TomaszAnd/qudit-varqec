...
def _kraus_diagonal(d: int, coupling_strengths: np.ndarray, eta: float, n_max: int) -> List[np.ndarray]:
    ...
    For each Kraus index n = 0, 1, ..., n_max:
        E_n[k,k] = (f_k * sqrt(-ln(eta)))^n * eta^{f_k^2 / 2} / sqrt(n!)

    where f_k is the coupling strength for level k. This is the Gaussian-averaged
    dephasing channel: for a random phase Phi ~ N(0, sigma_p^2) the coherence between
    two levels with coupling difference Delta_f decays as
        E[e^{i Delta_f Phi}] = e^{-1/2 sigma_p^2 Delta_f^2}
    (the standard Gaussian characteristic function), which these operators reproduce
    exactly, with Sum_n E_n^dag E_n = I (complete channel).
    ...
    # Gaussian-averaged dephasing (matches Meth arXiv:2310.12110v3 ground truth):
    # coherence between levels with coupling difference Delta_f decays as
    # e^{-1/2 sigma_p^2 Delta_f^2}. The paper's Appendix C carries an extra factor 2
    # (base=f*sqrt(2*sigma^2), decay=eta^{f^2}); that is a paper-side transcription
    # error to be fixed in the manuscript, NOT here -- this code follows Meth.
    base = coupling_strengths * np.sqrt(sigma_p2)  # f_k * sqrt(sigma_p^2)
    decay = eta ** (0.5 * coupling_strengths ** 2)  # eta^{f_k^2 / 2} = e^{-sigma_p^2 f_k^2 / 2}
...
def control_qudit_kraus_simplified(d: int, control_level: int, eta: float, n_max: int = 5) -> List[np.ndarray]:
    ...
    NOT a production path: this is the literal-Meth (uniform f=2) comparison
    variant, kept for cross-checking against the paper. The production noise
    functions use the physical, level-dependent control model instead.
...
def spectator_qudit_kraus(d: int, eta: float, n_max: int = 5) -> List[np.ndarray]:
    ...
    D_n = sum_k [k*sqrt(sigma_p^2)]^n * eta^{k^2 / 2} / sqrt(n!) * |k><k|
...
def amplitude_damping_kraus(d: int, gamma: float) -> List[np.ndarray]:
    """Qudit amplitude-damping Kraus operators (paper Eq. (6)).

        A_k = sum_r sqrt(C(r,k)) sqrt((1-gamma)^(r-k)) gamma^(k/2) |r-k><r|

    for k = 0..d-1. Idle decay toward |0> with per-event probability gamma
    (set by the experimental T1/T2 of Meth et al., App. A). Exactly
    trace-preserving for every gamma in [0, 1]: column r of sum_k A_k^dag A_k
    is the binomial sum ((1-gamma) + gamma)^r = 1. At gamma=0 the channel is
    the identity.

    Returns:
        List of d full d x d complex matrices.
    """
    ...
    for k in range(d):
        ...
        for r in range(k, d):
            A[r - k, r] = np.sqrt(comb(r, k)) * np.sqrt((1.0 - gamma) ** (r - k)) \
                * gamma ** (k / 2.0)
        ops.append(A)
    return ops
