...
def _auto_register_campaign_codes():
    ...
    Skip any npz already reachable under a human-readable name to avoid
    duplicate catalog entries (each .npz maps to exactly one code).
    """
    import glob
    import re
    existing_files = {v[0] for v in _CODE_REGISTRY.values() if v[0] is not None}
